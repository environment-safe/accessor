...
exports.setAll = exports.set = exports.getAll = exports.get = exports.augment = void 0;
function field(root, name, value, force) {
  ...
  while (name.length) {
    ...
    if (!current[fieldName]) {
      if (value) current[fieldName] = {};else return undefined;
    }
    if (!name.length) {
...
function allFields(root, name, value, force, parents) {
  ...
  while (name.length) {
    fieldName = name.shift();
    if (fieldName === '*') {
      let results = (current ? Object.keys(current) : []).reduce((res, k) => {
        let p = clone(parents);
        p.push(k);
...
const augment = function (target, data, wrap) {
...
const get = function (data, fieldName) {
  return field(data, fieldName);
};
exports.get = get;
const set = function (data, fieldName, value, force) {
  return field(data, fieldName, value, force);
};
exports.set = set;
const getAll = function (data, fieldName) {
  return allFields(data, fieldName);
};
exports.getAll = getAll;
const setAll = function (data, fieldName, value, force) {
